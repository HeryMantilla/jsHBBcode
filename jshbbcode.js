...
var jsHBBcode = {
    ...
    asc2bin: function(text) {
        ...
        var i = 0, enc = '', binary = [], len = 0;

        for (i = 0; i < 255; i += 1)  {
		    binary[this.chr(i)] = this.str_pad(this.base_convert(i, 10, 2), 8, '0', 'STR_PAD_LEFT');
        }
        ...
        enc = this.substr(this.chunk_split(enc, 8, ' '), 0, -1);
        return enc;
    ...
    asc2hex: function(text) {
        ...
        var i = 0, enc = '', hexadecimal = [], len = 0;

        for (i = 0; i < 255; i += 1) {
            hexadecimal[this.chr(i)] = this.str_pad(this.base_convert(i, 10, 16), 2, '0', 'STR_PAD_LEFT');
	    }
        ...
        enc = this.substr(this.chunk_split(enc, 2, ' '), 0, -1);
        return enc;
    ...
    base64_decode: function(str) {
        ...
        // Directly from Mozilla, for this purpose.
    ...
    bbencode_first_pass: function(text, uid) {
        ...
        text = self.bbencode_first_pass_pda(text, uid, '[code]', '[/code]', '', true, '');
        ...
        text = self.preg_replace("#\\[mrow\](.*?)#gm", "[mrow:${uid}]\\1", text);
        text = self.preg_replace("#\\[mrow color=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)#gm", "[mrow color=\\1:${uid}]\\2", text);
        text = self.preg_replace("#\\[mrow fontsize=([1-2]?[0-9])\](.*?)#gm", "[mrow fontsize=\\1:${uid}]\\2", text);
        ...
        text = self.preg_replace("#\\[mcol\](.*?)#gm", "[mcol:${uid}]\\1", text);
        text = self.preg_replace("#\\[mcol color=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)#gm", "[mcol color=\\1:${uid}]\\2", text);
        text = self.preg_replace("#\\[mcol fontsize=([1-2]?[0-9])\](.*?)#gm", "[mcol fontsize=\\1:${uid}]\\2", text);
        ...
        text = self.preg_replace("#\\[row\](.*?)#gm", "[row:${uid}]\\1", text);
        text = self.preg_replace("#\\[row color=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)#gm", "[row color=\\1:${uid}]\\2", text);
        text = self.preg_replace("#\\[row fontsize=([1-2]?[0-9])\](.*?)#gm", "[row fontsize=\\1:${uid}]\\2", text);
        ...
        text = self.preg_replace("#\\[col\](.*?)#gm", "[col:${uid}]\\1", text);
        text = self.preg_replace("#\\[col color=(\#[0-9A-F]{6}|[a-z\-]+)\](.*?)#gm", "[col color=\\1:${uid}]\\2", text);
        text = self.preg_replace("#\\[col fontsize=([1-2]?[0-9])\](.*?)#gm", "[col fontsize=\\1:${uid}]\\2", text);
        ...
        text = self.preg_replace("#\\[hr\]#gm", "[hr:${uid}]", text);

        text = self.preg_replace("#\\[myPost\]#gm", "[myPost:${uid}]", text);
    ...
    bbencode_first_pass_pda: function(text, uid, open_tag, close_tag, close_tag_new, mark_lowest_level, func, open_regexp_replace) {
        ...
        */
        ...
        if (open_regexp_replace === true || open_regexp_replace !== '') {
            open_is_regexp = true;
            ...
            if (curr_pos) {
                ...
                for (i = 0; i < open_tag_count; i += 1) {
                    ...
                    if ((this.preg_match('#\\[quote="#g', possible_start, 'match') && !this.preg_match('/\\[quote="(.*?)"\\]/g', possible_start)) ||
                        ...
                        (this.preg_match('#\\[tip="#g', possible_start, 'match') && !this.preg_match('/\\[tip="(.*?)"\\]/g', possible_start))
                    ) {
                        // OK we are in a quote tag that probably contains a ] bracket.
                        // Grab a bit more of the string to hopefully get all of it..
                        if (close_pos = this.strpos(text, '"]', curr_pos + 9)) {
                            if ((this.strpos(this.substr(text, curr_pos + 9, close_pos - (curr_pos + 9)), '[quote') === false) ||
                                (this.strpos(this.substr(text, curr_pos + 9, close_pos - (curr_pos + 9)), '[title') === false) ||
                                (this.strpos(this.substr(text, curr_pos + 9, close_pos - (curr_pos + 9)), '[mod') === false) ||
                                (this.strpos(this.substr(text, curr_pos + 9, close_pos - (curr_pos + 9)), '[tip') === false)
                            ) {
                ...
                if (found_start) {
                    ...
                    if (0 == this.strcasecmp(close_tag, possible_end)) {
                        ...
                        if (this.sizeof(stack) > 0) {
                            ...
                            if (use_function_pointer === true) { between_tags = func(between_tags, uid); }
    ...
    bbencode_second_pass: function(text, uid, logged, poster_posts) {
        ...
        // [tip] and [/tip]
        text = this.preg_replace('/\\[tip:${uid}="(.*?)"\]/si'.f({ uid }), bbcode_tpl['tip_open'], text);
        ...
        text = this.preg_replace("/\\[quote:${uid}=\"(.*?)\"\]/si".f({ uid }), bbcode_tpl['quote_username_open'], text);

        // Custom Title for Quote Boxes BBcode
        text = this.preg_replace("/\\[title:${uid}=\"(.*?)\"\]/si".f({ uid }), bbcode_tpl['free_txt_open'], text);
        ...
        // Ordered lists
        text = this.preg_replace('/\\[list=([a1]):${uid}\\]/si'.f({ uid }), bbcode_tpl['olist_open'], text);
        ...
        text = this.preg_replace('/\\[mod:${uid}="(.*?)"\]/si'.f({ uid }), bbcode_tpl['mod_username_open'], text);
        // END Moderator Tags
        ...
        text = this.str_replace("[h1:${uid}]".f({ uid }), '<span style="font-size: 24px; font-weight: bold;">', text);
        text = this.str_replace("[h2:${uid}]".f({ uid }), '<span style="font-size: 22px; font-weight: bold;">', text);
        text = this.str_replace("[h3:${uid}]".f({ uid }), '<span style="font-size: 20px; font-weight: bold;">', text);
        text = this.str_replace("[h4:${uid}]".f({ uid }), '<span style="font-size: 18px; font-weight: bold;">', text);
        text = this.str_replace("[h5:${uid}]".f({ uid }), '<span style="font-size: 16px; font-weight: bold;">', text);
        text = this.str_replace("[h6:${uid}]".f({ uid }), '<span style="font-size: 14px; font-weight: bold;">', text);

        // [table] and [/table] for making tables.
        // beginning code [table], along with attributes
        text = this.str_replace("[table:${uid}]".f({ uid }), bbcode_tpl['table_open'], text);
        text = this.preg_replace("/\\[table color=(\#[0-9A-F]{6}|[a-z]+):${uid}\]/si".f({ uid }), bbcode_tpl['table_color'], text);
        text = this.preg_replace("/\\[table fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_size'], text);
        text = this.preg_replace("/\\[table color=(\#[0-9A-F]{6}|[a-z]+) fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_cs1'], text);

        // mainrow tag [mrow], along with attributes
        text = this.str_replace("[mrow:${uid}]".f({ uid }), bbcode_tpl['table_mainrow'], text);
        text = this.preg_replace("/\\[mrow color=(\#[0-9A-F]{6}|[a-z]+):${uid}\]/si".f({ uid }), bbcode_tpl['table_mainrow_color'], text);
        text = this.preg_replace("/\\[mrow fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_mainrow_size'], text);
        text = this.preg_replace("/\\[mrow color=(\#[0-9A-F]{6}|[a-z]+) fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_mainrow_cs1'], text);

        // maincol tag [mcol], along with attributes
        text = this.str_replace("[mcol:${uid}]".f({ uid }), bbcode_tpl['table_maincol'], text);
        text = this.preg_replace("/\\[mcol color=(\#[0-9A-F]{6}|[a-z]+):${uid}\]/si".f({ uid }), bbcode_tpl['table_maincol_color'], text);
        text = this.preg_replace("/\\[mcol fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_maincol_size'], text);
        text = this.preg_replace("/\\[mcol color=(\#[0-9A-F]{6}|[a-z]+) fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_maincol_cs1'], text);

        // row tag [row], along with attributes
        text = this.str_replace("[row:${uid}]".f({ uid }), bbcode_tpl['table_row'], text);
        text = this.preg_replace("/\\[row color=(\#[0-9A-F]{6}|[a-z]+):${uid}\]/si".f({ uid }), bbcode_tpl['table_row_color'], text);
        text = this.preg_replace("/\\[row fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_row_size'], text);
        text = this.preg_replace("/\\[row color=(\#[0-9A-F]{6}|[a-z]+) fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_row_cs1'], text);

        // column tag [col], along with attributes
        text = this.str_replace("[col:${uid}]".f({ uid }), bbcode_tpl['table_col'], text);
        text = this.preg_replace("/\\[col color=(\#[0-9A-F]{6}|[a-z]+):${uid}\]/si".f({ uid }), bbcode_tpl['table_col_color'], text);
        text = this.preg_replace("/\\[col fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_col_size'], text);
        text = this.preg_replace("/\\[col color=(\#[0-9A-F]{6}|[a-z]+) fontsize=([1-2]?[0-9]):${uid}\]/si".f({ uid }), bbcode_tpl['table_col_cs1'], text);

        // ending tag [/table]
        text = this.str_replace("[/table:${uid}]".f({ uid }), bbcode_tpl['table_close'], text);

        text = this.preg_replace("#\[/h[1-6]:${uid}\]#gm".f({ uid }), '</span>', text);
        ...
        // [ytube]youtube_url_here[/ytube] code..
        patterns.push("#\\[ytube\\]http:\\/\\/(?:www\\.)?youtube.com\\/watch\\?v=([0-9A-Za-z-_]{11})[^[]*\\[\\/ytube\\]#gm");
        replacements.push(bbcode_tpl['ytube']);

        patterns.push("#\\[ytube\\]([\\w]+?:\\/\\/([\\w\\#$%&~/.\\-;:=,?@\\]+]+|\\[(?!url=))*?)\\[\\/ytube\\]#gm");
        replacements.push(bbcode_tpl['ytube']);
        ...
        patterns.push("#\\[wiki\\](.*?)\\[\\/wiki\\]#gm");
        replacements.push(bbcode_tpl['wiki']);

        if (bbcode_tpl['wiki_es'] !== '\'undefined\'') {
            patterns.push("#\\[wiki=es\\](.*?)\\[\\/wiki\\]#gm");
            replacements.push(bbcode_tpl['wiki_es']);
        }
        ...
        text = this.preg_replace("#\\[reverse:${uid}\\](.*?)\\[\\/reverse:${uid}\\]#gm".f({ uid }), "strrev('\\1')", text);

        // [rainbow] for rainbow-highlighting text
        text = this.preg_replace("#\\[rainbow:${uid}\\](.*?)\\[\\/rainbow:${uid}\\]#gm".f({ uid }), "rainbow('\\1')", text);
    ...
    bin2asc: function(text) {
        ...
        text = this.str_replace(' ', ':::NULLNULLNULL:::', text);
        binary.foreach(function(char, bin) {

            text = this.str_replace(bin, char, text);

        });

        return this.str_replace(':::NULLNULLNULL:::', '', text);
    ...
    hex2asc: function(text) {
        ...
        text = str_replace(' ', ':::NULLNULLNULL:::', text);
        hexadecimal.foreach(function(char, hex) {

            text = this.str_replace(hex, char, text);
    ...
    parser: function(str) {
        ...
        rand_seed = 'bb_' + this.substr(this.rand(), 0, 8);
        ...
        if (typeof str !== 'undefined') {
            ...
            result = this.bbencode_second_pass(this.bbencode_first_pass(str, rand_seed, false, 0), rand_seed, true, 0);
    ...
    preg_match: function(pattern, str, match) {
        ...
        if ((window[match] === null || typeof window[match] === 'undefined') && (typeof match !== 'undefined')) { window[match] = []; }
        ...
        try {
            ...
            while ((m = regex.exec(str)) !== null) {
                ...
                if (m && (typeof match !== 'undefined')) { window[match].push(m); }
            }

            return (typeof match !== 'undefined') ? window[match].length : 0;
        } catch {
            return 0;
        }
    ...
    preg_match_all: function(pattern, str, matches) {
        ...
        if (window[matches] === null || typeof window[matches] === 'undefined' && (typeof matches !== 'undefined')) { window[matches] = []; }
        ...
        try {
            ...
            while ((m = regex.exec(str)) !== null) {
                ...
                if (m && (typeof matches !== 'undefined')) { window[matches].push(m); }
            }

            return (typeof matches !== 'undefined') ? window[matches].length : 0;
        } catch {
            return 0;
        }
        ...
        try {
            ...
            for (x = res.length -1; x > -1; x--) {
                ...
                for (y = res[x].length - 1; y > -1; y--) {
                    ...
                    tmp = tmp.replace('${' + y + '}', row).replace('$' + y, row).replace('\\' + y, row);
                }
    ...
    prepare_bbcode_template: function(bbcode_tpl) {
        ...
        bbcode_tpl['free_txt_open'] = this.str_replace('{USERNAME}', '\\1', bbcode_tpl['free_txt_open']);
        ...
        bbcode_tpl['wiki'] = this.str_replace('{STRING}', "'" + this.str_replace('\\\"', '\"', '\\1') + "'", bbcode_tpl['wiki']);
        bbcode_tpl['wiki'] = this.str_replace('{QUERY}', "'" + this.urlencode(this.str_replace('\\\"', '\"', '\\1')) + "'", bbcode_tpl['wiki']);

        bbcode_tpl['wiki_es'] = '\'' + bbcode_tpl['wiki_es'] + '\'';
        bbcode_tpl['wiki_es'] = this.str_replace('{STRING}', "'" + this.str_replace('\\\"', '\"', '\\1') + "'", bbcode_tpl['wiki_es']);
        bbcode_tpl['wiki_es'] = this.str_replace('{QUERY}', "'" + this.urlencode(this.str_replace('\\\"', '\"', '\\1')) + "'", bbcode_tpl['wiki_es']);
    ...
    rainbow: function(text) {
        ...
        var colors = this.load_rainbow_colors(),
            length = 0,
            result = '',
            color_counter = 0,
            TAG_OPEN = false;

        text = this.trim(stripslashes(text));
        length = this.strlen(text);

        for (var i = 0; i < length; i += 1) {
            var char = this.substr(text, i, 1);

            if (TAG_OPEN === false) {
                if (char == '<') {
                    TAG_OPEN = true;
                    result += char;
                } else if (this.preg_match('#\S#i', char)) {
                    color_counter++;
    ...
    smilies_pass: function(message) {
        ...
        for (i = 0; i < lCount; i += 1) {
			orig.push("/" + this.preg_quote(smilies[i]['code'], "/") + "/g");
